...
const notifyAuthor = async (supabase: any, commentId: number, threadId: number, userId: string, type: 'COMMENT' | 'REPLY') => {
  const { data: authorData } = await supabase.from('Comment').select('author_id').eq('comment_id', commentId).single();

  if (authorData && authorData.author_id !== userId) {
    await createCommentNotification(authorData.author_id, userId, threadId, commentId, type);
  }
};
...
export function useComments(initialThread: Thread, user: any | null): CommentState & CommentHandlers {
  ...
  const handleReplySubmit = async (parentCommentId: number, content?: string) => {
    ...
    try {
      ...
      await notifyAuthor(supabase, data.comment_id, thread.thread_id, user.id, 'REPLY');
