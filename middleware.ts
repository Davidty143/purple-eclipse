...
const protectedRoutes = ['/dashboard', '/admin/settings', '/protected', '/login', '/set-username', '/register', '/signup', '/post-thread', '/profile', '/notifications', '/messages', '/update-password'];
...
export const middleware = async (request: NextRequest) => {
  ...
  // Get the requested route path and search params
  ...
  // Allow /update-password access if the request includes recovery token params
  if (pathname === '/update-password' && searchParams.has('token') && searchParams.get('type') === 'recovery') {
