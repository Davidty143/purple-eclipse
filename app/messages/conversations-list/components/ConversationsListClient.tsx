...
export default function ConversationsListClient({ userId }: { userId: string }) {
      ...
      <div className="space-y-1">
        {partners.length === 0 ? (
          ...
          partners.map((partner) => (
            <Link key={partner.id} href={`/messages/${partner.id}`} className={cn('flex items-center p-3 rounded-lg transition-colors hover:bg-muted/50')}>
